...
2. **Run the Example**: Enter the command:
   ```shell
   go run infini.sh/coco/lib/langchaingo/examples/ollama-completion-example@main
   ```
...
Here is the entire program (from [ollama-completion-example](https://infini.sh/coco/lib/langchaingo/blob/main/examples/ollama-completion-example/ollama_completion_example.go)).
