...
func (p *FileExtractionProcessor) processPdf(ctx context.Context, doc *core.Document) ([]string, []string, error) {
	...
	if err != nil {
		return nil, nil, fmt.Errorf("failed to extract text for [%s] using tika: %w", path, err)
	}
	...
	if err != nil {
		return nil, nil, fmt.Errorf("failed to parse tika response for %s: %w", path, err)
	}

	attachmentDirPath, err := os.MkdirTemp("", "attachment-temp-")
	if err != nil {
		return nil, nil, fmt.Errorf("failed to create temporary directory for extracting document attachments: %w", err)
	}
	defer os.RemoveAll(attachmentDirPath)

	err = tikaUnpackAllTo(tikaRequestCtx, p.config.TikaEndpoint, path, attachmentDirPath)
	if err != nil {
		return nil, nil, fmt.Errorf("failed to extracte document attachments: %w", err)
	}
	...
	*/
	...
	for i := 0; i < pagesSelection.Length(); i++ {
		...
		p.appendPage(tikaRequestCtx, s, attachmentDirPath, &pagesWithoutOcr, &pagesWithOcr)
	}
...
func (p *FileExtractionProcessor) appendPage(tikaRequestCtx context.Context, s *goquery.Selection, attachmentDir string, pagesWithoutOcr, pagesWithOcr *[]string) {
	// Clone s because goquery modifies nodes in-place.
	sClone := s.Clone()

	/*
	 * append to [pagesWithoutOcr]
	 */
