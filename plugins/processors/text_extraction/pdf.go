...
func (p *TextExtractionProcessor) processPdf(ctx context.Context, doc *core.Document, localPath string) (fileproc.Extraction, error) {
...
func (p *TextExtractionProcessor) appendPage(s *goquery.Selection, pageNum int, nameToId map[string]string, nameToText map[string]string, nameToPageNums map[string][]int, pages *[]string) {
	s.Find("img").Each(func(i int, img *goquery.Selection) {
		...
		if exists {
			...
			if !ok {
				panic(fmt.Sprintf("unreachable: attachment ID not found for file %s", imageName))
			}
