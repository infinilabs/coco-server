 ...
 */
export function getColorPalette(color: string, recommended = false) {
  const colorMap = new Map();

  if (recommended) {
    const colorPalette = getRecommendedColorPalette(getHex(color));
    colorPalette.palettes.forEach((palette: any) => {
 ...
 * @param recommended whether to get recommended color palette (the provided color may not be the main color)
 */
export function getPaletteColorByNumber(color: string, number: number, recommended = false) {
