...
export function getRecommendedColorPalette(color: string) {
  ...
  colorPaletteFamily.palettes.forEach((palette: any) => {
    colorMap.set(palette.number, palette);
  });

  const mainColor = colorMap.get(500);
  const matchColor = colorPaletteFamily.palettes.find((palette: any) => palette.hex === color);
 ...
 */
export function getRecommendedPaletteColorByNumber(color: string, number: number) {
...
export function getRecommendedColorPaletteFamily(color: string) {
  ...
  const colorPaletteFamily = {
    name: colorName,
    palettes: palettes.map((palette: any) => {
      let hexValue = color;
 ...
 */
function getNearestColorPaletteFamily(color: string, families: any[]) {
  const familyWithConfig = families.map((family: any) => {
    const palettes = family.palettes.map((palette: any) => {
      return {
        ...palette,
    ...
    const nearestPalette = palettes.reduce((prev: any, curr: any) => (prev.delta < curr.delta ? prev : curr));
  ...
  const nearestPaletteFamily = familyWithConfig.reduce((prev: any, curr: any) =>
  ...
  const paletteFamily = {
    ...
    nearestLightnessPalette: nearestPaletteFamily.palettes.reduce((prev: any, curr: any) => {
