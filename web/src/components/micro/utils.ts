export function getEndpoint() {
  return window.__POWERED_BY_WUJIE__ && window.$wujie?.props?.endpoint ? window.$wujie?.props?.endpoint : `${window.location.origin}${window.location.pathname === '/' ? '' : window.location.pathname}`;
}
