const page: App.I18n.Schema['translation']['page'] = {
  ...
  assistant: {
    ...
    labels: {
      ...
      deep_research_models: '模型设置',
      planning_model: '规划模型',
      research_model: '研究模型',
      synthesis_model: '综合模型',
      report_model: '报告模型',
      deep_research_limits: '研究设置',
      max_steps: '最大研究步数',
      max_steps_desc: '研究流程中的最大迭代步骤数',
      max_researcher_iterations: '最大研究迭代',
      max_researcher_iterations_desc: '单个研究步骤的最大重试次数',
      max_results: '每次搜索最大结果数',
      timeout: '超时时间',
      timeout_desc: '研究任务的总超时时间（如 30m、1h）',
      timeout_invalid: '格式非法，请输入合法格式，如 30m、1h、1h30m',
      research_depth: '研究深度',
      research_depth_basic: '基础 — 快速概览，适合简单问题',
      research_depth_comprehensive: '综合 — 深度分析，适合复杂问题',
      research_depth_exhaustive: '详尽 — 全面研究，适合深度主题',
      deep_research_output: '输出设置',
      ...
      report_lang_zh: '中文',
      report_lang_en: '英文',
      deep_research_search: '搜索设置',
      datasource_search: '数据源搜索',
      datasource_ids_placeholder: '不限定（使用所有可访问数据源）',
      external_search_engine: '搜索引擎',
      search_engine_wikipedia: '维基百科',
      external_search_api_key: 'API Key',
      tavily_api_key_placeholder: '请输入 Tavily API Key',
  ...
  datasource: {
    ...
    milvus: {
      error: {
        address_required: '请输入 Milvus 服务地址！',
        collection_required: '请输入集合名称！',
        page_size_required: '请输入页面大小！',
      ...
      labels: {
        ...
        resume_from: '起始值',
        tie_breaker: '并列项字段',
        username_placeholder: '可选的 Milvus 用户名',
        password_placeholder: '可选的 Milvus 密码',
        db_name_placeholder: '可选的数据库名称'
      },
      tooltip: {
        address: 'Milvus 服务地址，例如：localhost:19530',
        username: '可选的 Milvus 认证用户名',
        password: '可选的 Milvus 认证密码',
        db_name: '可选的数据库名称（Milvus 2.2.0+）。留空表示使用默认数据库。',
        collection: '要查询的 Milvus 集合名称',
        output_fields: '逗号分隔的字段列表。留空表示检索所有标量字段。',
        filter: '可选的标量过滤表达式，例如：age > 10 and name like "abc%"',
        page_size: '每页获取的文档数（1-10000）',
        pagination: '为大型集合启用分页',
        incremental_property: '作为水位的 Milvus 字段。每个向量都必须包含该字段。',
        incremental_property_type: '选择水位属性的数据类型，以确保排序稳定。',
        incremental_resume_from: '可选的首次运行起始水位值。',
        incremental_sync: '启用后，将使用水位属性从上次进度继续扫描。',
        incremental_tie_breaker: '用于区分具有相同水位值记录的字段，例如 id。',
        property: '用于跟踪变更的字段名',
        property_type: '跟踪属性的数据类型，用于正确比较',
        resume_from: '可选的首次同步手动起始点',
        tie_breaker: '当多个文档具有相同属性值时，用于打破平局的次要字段'
      }
    },
    neo4j: {
      error: {
  ...
  integration: {
    ...
    form: {
      labels: {
        ...
        datasource: '数据源',
        deep_research_assistant: '深度研究小助手',
        deep_think_assistant: '深度思考小助手',
        description: '描述',
        enable_auth: '启用认证',
        tourist_mode: '游客模式',
