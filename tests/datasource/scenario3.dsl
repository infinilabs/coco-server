#// Scenario 3:
#//
#// Users that are granted share permission can share datasource/document, then
#// revoke their grants.
...
#// 7
#//
#// Admin grants user [b] [Share] access to [datasource_1:file_b]
...
#//   2. User [a] revokes the permission perviously granted
#//   3. User [b] cannot see [datasource_2], it can only see [datasource_1]
#//   4. User [c] don't have access to [datasource_2:file_a], it can only see [datasource_2:file_c]
#//   5. User [d] don't have access to [datasource_1, datasource_2]
#//
#//----------------------------------------------------------------------------

#// 13
#//
#// User a grant user [b] [view] permission to [datasource_2:file_a]
POST /resources/document/$[[env.DATASOURCE_2_DOCUMENT_FILE_A_ID]]/share
{
  "shares": [
    {
...
#
# register: [
#   { grant_user_a_doc_2a_user_b: "_ctx.response.body_json.created.0.id" },
# ],
#
# assert: (200, {
#   "created": [
...
#// 14
#//
#// User [a] grants user [c] [edit] permission to [datasource_2:file_a]
POST /resources/document/$[[env.DATASOURCE_2_DOCUMENT_FILE_A_ID]]/share
{
  "shares": [
    {
...
#
# register: [
#   { grant_user_a_doc_2a_user_c: "_ctx.response.body_json.created.0.id" },
# ],
#
# assert: (200, {
#   "created": [
...
#// 15
#//
#// User [a] revokes the [share] permission granted to user [b]
POST /resources/document/$[[env.DATASOURCE_2_DOCUMENT_FILE_A_ID]]/share
{
  "revokes": [
    {
      "id": "$[[grant_user_a_doc_2a_user_b]]",
      "principal_type": "user",
      "principal_id": "$[[env.B_ID]]",
      "permission": $[[env.SHARE_ACCESS]]
    }
...
#// 16
#//
#// User [a] revokes the [edit] permission granted to user [c]
POST /resources/document/$[[env.DATASOURCE_2_DOCUMENT_FILE_A_ID]]/share
{
  "revokes": [
    {
      "id": "$[[grant_user_a_doc_2a_user_c]]",
      "principal_type": "user",
      "principal_id": "$[[env.C_ID]]",
      "permission": $[[env.EDIT_ACCESS]]
    }
...
#// 17
#//
#// User [b] cannot see [datasource_2], it can only see [datasource_1]
GET /datasource/_search
# request: {
#   headers: [
#     {Authorization: "Bearer $[[b_token]]"},
...
# assert: (200, {
#   "hits.total.value": 1,
#   "hits.hits": [ { "_source.name": "$[[env.DATASOURCE_1_NAME]]"  } ]
# })
#
...
#// 18
#//
#// User [c] don't have access to [datasource_2:file_a], it can only see [datasource_2:file_c]
...
# assert: (200, {
#   "hits.total.value": 1,
#   "hits.hits": [ { "_source.title": "file_c"  } ]
# })
...
#//   2. User [b] revokes the permission perviously granted
#//   3. User [a] cannot see [datasource_1], it can only see [datasource_2]
#//   4. User [c] cannot see [datasource_1], it can only see [datasource_2]
#//   5. User [d] don't have access to [datasource_1, datasource_2]
#//
#//----------------------------------------------------------------------------
...
#// 20
#//
#// User [b] grants user [a] [view] permission to [datasource_1:file_b]
POST /resources/document/$[[env.DATASOURCE_1_DOCUMENT_FILE_B_ID]]/share
{
  "shares": [
    {
...
#
# register: [
#   { grant_user_b_doc_1b_user_a: "_ctx.response.body_json.created.0.id" },
# ],
#
# assert: (200, {
#   "created": [
...
#// 21
#//
#// User [b] grants user [c] [edit] permission to [datasource_1:file_b]
POST /resources/document/$[[env.DATASOURCE_1_DOCUMENT_FILE_B_ID]]/share
{
  "shares": [
    {
...
#
# register: [
#   { grant_user_b_doc_1b_user_c: "_ctx.response.body_json.created.0.id" },
# ],
#
# assert: (200, {
#   "created": [
...
#// 22
#//
#// User [b] revokes the [view] permission granted to user [a]
POST /resources/document/$[[env.DATASOURCE_1_DOCUMENT_FILE_B_ID]]/share
{
  "revokes": [
    {
      "id": "$[[grant_user_b_doc_1b_user_a]]",
      "principal_type": "user",
      "principal_id": "$[[env.A_ID]]",
      "permission": $[[env.VIEW_ACCESS]]
    }
...
#// 23
#//
#// User [b] revokes the [edit] permission granted to user [c]
POST /resources/document/$[[env.DATASOURCE_1_DOCUMENT_FILE_B_ID]]/share
{
  "revokes": [
    {
      "id": "$[[grant_user_b_doc_1b_user_c]]",
      "principal_type": "user",
      "principal_id": "$[[env.C_ID]]",
      "permission": $[[env.EDIT_ACCESS]]
    }
...
#// 24
#//
#// User [a] cannot see [datasource_1], it can only see [datasource_2]
GET /datasource/_search
# request: {
#   headers: [
#     {Authorization: "Bearer $[[a_token]]"},
...
# assert: (200, {
#   "hits.total.value": 1,
#   "hits.hits": [ { "_source.name": "$[[env.DATASOURCE_2_NAME]]"  } ]
# })


#// 25
#//
#// User [c] cannot see [datasource_1], it can only see [datasource_2]
GET /datasource/_search
# request: {
#   headers: [
#     {Authorization: "Bearer $[[c_token]]"},
...
#//   2. User [c] revokes the permission perviously granted
#//   3. User [a] can only see [datasource_2:file_a], it can only see [datasource_2:file_c]
#//   4. User [b] cannot see [datasource_2], it can only see [datasource_1]
#//   5. User [d] don't have access to [datasource_1, datasource_2]
#//
#//----------------------------------------------------------------------------
...
#// 27
#//
#// User [c] grants user [a] [view] permission to [datasource_2:file_c]
POST /resources/document/$[[env.DATASOURCE_2_DOCUMENT_FILE_C_ID]]/share
{
  "shares": [
    {
...
#
# register: [
#   { grant_user_c_doc_2c_user_a: "_ctx.response.body_json.created.0.id" },
# ],
#
# assert: (200, {
#   "created": [
...
#// 28
#//
#// User [c] grants user [b] [edit] permission to [datasource_2:file_c]
POST /resources/document/$[[env.DATASOURCE_2_DOCUMENT_FILE_C_ID]]/share
{
  "shares": [
    {
...
#
# register: [
#   { grant_user_c_doc_2c_user_b: "_ctx.response.body_json.created.0.id" },
# ],
#
# assert: (200, {
#   "created": [
...
#// 29
#//
#// User [c] revokes the [view] permission granted to user [a]
POST /resources/document/$[[env.DATASOURCE_2_DOCUMENT_FILE_C_ID]]/share
{
  "revokes": [
    {
      "id": "$[[grant_user_c_doc_2c_user_a]]",
      "principal_type": "user",
      "principal_id": "$[[env.A_ID]]",
      "permission": $[[env.VIEW_ACCESS]]
    }
...
#// 30
#//
#// User [c] revokes the [edit] permission granted to user [b]
POST /resources/document/$[[env.DATASOURCE_2_DOCUMENT_FILE_C_ID]]/share
{
  "revokes": [
    {
      "id": "$[[grant_user_c_doc_2c_user_b]]",
      "principal_type": "user",
      "principal_id": "$[[env.B_ID]]",
      "permission": $[[env.EDIT_ACCESS]]
    }
...
#// 31
#//
#// User [a] can only see [datasource_2:file_a]
GET /document/_search?filter=source.id:any($[[env.DATASOURCE_2_ID]])&from=0&size=100&query=file
# request: {
#   headers: [
#     {Authorization: "Bearer $[[a_token]]"},
